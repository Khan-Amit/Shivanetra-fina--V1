...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    db_user = models.User(
        email=user.email,
        password_hash=user.password_hash,  # hash before storing!
...
def save_natal_chart(db: Session, profile_id: int, chart_data: dict, house_system: str = "Placidus"):
    """Store a complete calculated chart."""
...
def get_latest_chart(db: Session, profile_id: int):
    """Retrieve the most recent chart for a profile."""
...
def get_cached_planet(db: Session, body_id: int, julian_day: float):
    """Retrieve a single cached planet position."""
